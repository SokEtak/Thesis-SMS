...
const attendanceMatchesSearch = (item: Attendance, term: string): boolean => {
...
export default function Trashed({ attendances, query }: Props) {
  ...
  const filteredRows = useMemo(() => (
    attendances.data.filter((item) => attendanceMatchesSearch(item, searchValue))
  ), [attendances.data, searchValue]);
  ...
  const selectedAttendances = useMemo(() => {
    ...
    return attendances.data.filter((item) => selectedSet.has(item.id));
  ...
  const visitPage = (page: number, perPage = activePerPage) => {
    ...
    router.get(route('attendances.trashed', nextQuery), {}, {
      preserveState: true,
      preserveScroll: true,
      replace: true,
  ...
  const handleRestore = async (attendance: Attendance) => {
    ...
    router.post(route('attendances.restore', attendance.id), {}, { preserveScroll: true });
  };
  ...
  const handleForceDelete = async (attendance: Attendance) => {
    ...
    router.delete(route('attendances.forceDelete', attendance.id), { preserveScroll: true });
  };
  ...
  const handleBatchRestore = async () => {
    ...
    router.post(route('attendances.batchRestore'), { ids: selectedIds }, {
      preserveScroll: true,
  ...
  const handleBatchForceDelete = async () => {
    ...
    router.post(route('attendances.batchForceDelete'), { ids: selectedIds }, {
      preserveScroll: true,
      onSuccess: () => {
          ...
          actions={[
            {
              ...
              iconOnly: true,
              variant: 'outline',
              onClick: (row: Attendance) => openViewModal(row),
            },
            {
              key: 'restore',
              label: 'Restore',
              ...
              iconOnly: true,
              variant: 'outline',
              onClick: (row: Attendance) => void handleRestore(row),
            },
            {
              key: 'force-delete',
              ...
              iconOnly: true,
              variant: 'outline',
              onClick: (row: Attendance) => void handleForceDelete(row),
            },
          ]}
